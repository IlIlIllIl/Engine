...
local GuiLibrary = {
    ["getRobloxAsset"] = function(path) 
        ...
        if name == "arrow.png" then
            return "rbxassetid://53411462" 
        elseif name == "gear.png" then
            return "rbxassetid://9223902776"
        elseif name == "click.mp3" then 
            return "rbxassetid://6042054037"
        end
    end,
...
makefolder("Valkyrie")
makefolder("Valkyrie/logs")
makefolder("Valkyrie/assets")
makefolder("Valkyrie/configs")
makefolder("Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId))
makefolder("Valkyrie/configs/!SelectedConfigs/")
...
local function playclicksound() 
    if GuiLibrary["ClickSounds"] then
        playsound(getasset("Valkyrie/assets/click.mp3"))
    end
end
...
GuiLibrary["PrepareWatermark"] = function()
    ...
    function api.draw() 
        ...
        Watermark.Text = "Valkyrie.uno | Build : Premium | Version : 2.0 "
...
GuiLibrary["CreateArrayList"] = function() 
    ...
    function api.createArrayObject(name,label)
        ...
        ArrayObject.Size = UDim2.new(0, 0, 0, 18)
...
GuiLibrary["SaveConfig"] = function(name, isAutosave) 
    ...
    GuiLibrary["Debug"]("save Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..name..".json")
    local config = {}
    ...
    local path = "Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..name..".json"
    makefolder("Valkyrie/configs")
    makefolder("Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId))
    ...
    repeat task.wait() until betterisfile((path))
    if betterisfile("Valkyrie/configs/GUIconfig.json") then 
        delfile("Valkyrie/configs/GUIconfig.json")
    end
    writefile("Valkyrie/configs/GUIconfig.json", HTTPSERVICE:JSONEncode(guiconfig))
    repeat task.wait() until betterisfile("Valkyrie/configs/GUIconfig.json")
end
GuiLibrary["LoadOnlyGuiConfig"] = function() 
    if betterisfile("Valkyrie/configs/GUIconfig.json") then 
        local success, config = pcall(function() 
            local x = readfile("Valkyrie/configs/GUIconfig.json")
        ...
        if success then 
            ...
            warn("[Valkyrie] Failed to load GUIconfig.json config file")
        end
    else
        for i,v in next, GuiLibrary.Objects do 
...
GuiLibrary["LoadConfig"] = function(name) 
    ...
    writefile("Valkyrie/configs/!SelectedConfigs/"..tostring(shared.FuturePlaceId or game.PlaceId)..".txt", name) 
    GuiLibrary["Debug"]("Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..name..".json")
    if betterisfile("Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..name..".json") then 
        print("[Valkyrie] Loading configuration "..name)
        log("LoadConfig", "Loading "..name)
        local success, config = pcall(function() 
            local x = readfile("Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..name..".json")
            return HTTPSERVICE:JSONDecode(x)
        end)
        if success then 
            print("[Valkyrie] Success loading configuration "..name)
            ...
            warn("[Valkyrie] Failed to load "..tostring(shared.FuturePlaceId or game.PlaceId)..".json config file\nplease report this in the discord!\n("..config..")")
        end
    end
        ...
        buttonapi["CreateSelector"] = function(argstable) 
            ...
            selectorapi["SelectNext"] = function() 
                ...
                if newindex then 
                    ...
                    warn("[Valkyrie] NewIndex in selector ("..argstable.Name..") in function `SelectNext` was not found!")
                end
                playclicksound()
            end
