...
if table.find(UserNames, tostring(player.Name)) then
    player:Kick("Your account is currently banned.\nBan Reason: undermining\n Ban will be lifted: Never")
else
print("[Valkyrie] Loading!")
repeat task.wait() until game:IsLoaded()
if shared.Future~=nil then print("[Valkyrie] Detected future already executed, not executing!") return end
shared.futureStartTime = game:GetService("Workspace"):GetServerTimeNow()
shared._FUTUREVERSION = "2.0 | "..((shared.FutureDeveloper and "dev" or "version")).." Donator" -- // This is a cool thing yes
shared._FUTUREMOTD = "2022 RECODE 🔥"
...
local function requesturl(url, bypass) 
    ...
    local repourl = bypass and "https://raw.githubusercontent.com/IlIlIllIl/" or "https://raw.githubusercontent.com/IlIlIllIl/Engine/main/"
    local url = url:gsub("Engine/", "")
...
if not suc then 
    warn("[Valkyrie] AntiPreloadAsync failed to load: "..err)
end
...
local GuiLibrary = loadstring(requesturl("Engine/GuiLibrary.lua"))()
...
local function getasset(path)
	if not betterisfile(path) then
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/IlIlIllIl/Engine/main/"..path:gsub("Engine/assets", "assets"),
			Method = "GET"
		})
        print("[Valkyrie] downloading "..path.." asset.")
		writefile(path, req.Body)
        repeat task.wait() until betterisfile(path)
        print("[Valkyrie] downloaded "..path.." asset successfully!")
	end
...
local function getscript(id) 
    ...
    id = tostring(id)
    local req = requesturl("Engine/scripts/"..id..".lua")
...
local function getplusscript(id) -- future plus moment
    local id = id or shared.FuturePlaceId or game.PlaceId
    id = tostring(id)
    if not betterisfile("Engine/plus/"..id..".fp") then return end
    local req = readfile("Engine/plus/"..id..".fp")
    if type(req) == "string" then
        return loadstring(req)()
    else
        --fwarn("[Future] invalid script (error "..tostring(req)..")") -- game is not supported
...
local function getcustomscripts(id) 
    local id = id or shared.FuturePlaceId or game.PlaceId
    id = tostring(id)
    if not isfolder("Engine/custom-scripts/"..id) then 
        return
    end
    local files = listfiles("Engine/custom-scripts/"..id)
    for i,v in next, files do 
        local req = readfile(v)
        if type(req) == "string" then
            print("[Valkyrie] Loading script ", v)
            loadstring(req)()
        end
    end
end
...
local friendstab = {pcall(function() HTTPSERVICE:JSONDecode(readfile("Engine/Friends.json")) end)}
...
local configBox; configBox = configButton.CreateTextbox({
    ...
    ["Function"] = function(value)
        spawn(function()
            ...
            if betterisfile("Valkyrie/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..value..".json") then
...
if betterisfile("Valkyrie/configs/!SelectedConfigs/"..tostring(shared.FuturePlaceId or game.PlaceId)..".txt") then 
    GuiLibrary.CurrentConfig = readfile("Valkyrie/configs/!SelectedConfigs/"..tostring(shared.FuturePlaceId or game.PlaceId)..".txt") 
    configBox.Set(GuiLibrary.CurrentConfig, true)
    print("[Valkyrie] Detected config ",GuiLibrary.CurrentConfig," used last time!")
end
...
--[[local discordButton = {["Toggle"] = function(...) end} discordButton = OtherWindow.CreateOptionsButton({
    ...
    ["Function"] = function(callback)
        if callback then
            pcall(function() setclipboard("https://discord.gg/TmJkAbEtys") end)
            spawn(function()
				for i = 1, 14 do
					spawn(function()
						local reqbody = {
        ...
        if callback then 
            spawn(function() 
                ...
                if shared.FutureDeveloper then 
                    ...
                    loadstring(game:HttpGet('https://raw.githubusercontent.com/IlIlIllIl/Engine/main/loadstring.lua', true))()
                end
            end)
        end
    end
...
commands.friend = {
    Function = function(args) 
        ...
        writefile("Engine/Friends.json", HTTPSERVICE:JSONEncode(Future.Friends))
    end,
...
local ontp = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
        ...
        if shared.FutureDeveloper then 
            ...
            loadstring(game:HttpGet("https://raw.githubusercontent.com/IlIlIllIl/Engine/main/Initiate.lua", true))() 
        end
        ]]
...
if success then 
    print("[Valkyrie] Successfully retrieved Universal script!")
else
...
if success2 then 
    print("[Valkyrie] Successfully retrieved Game script!")
else
...
if success3 then 
    print("[Valkyrie] Successfully retrieved required Game script!")
else
    fwarn("Unsuccessful attempt at retrieving required Game script!\n (".._error3..")")
    GuiLibrary.Signals.onDestroy:Fire()
    return
end
if success4 then 
    print("[Valkyrie] Successfully loaded all custom scripts!")
else
...
if not suc then warn("[Valkyrie] Chat hook failed, aborting command system. \n(Error: "..res..")") end
...
GuiLibrary.Signals.onDestroy:connect(function()
    ...
    writefile("Valkyrie/configs/!SelectedConfigs/"..tostring(shared.FuturePlaceId)..".txt", GuiLibrary.CurrentConfig) 
...
spawn(function()
    if GuiLibrary["AllowNotifications"] then
        ...
        textlabel.Text = [[<stroke thickness="2">Valkyrie.uno loaded, Have fun.</stroke>]]
        ...
        textlabel2.Text = [[<stroke thickness="2">V2.0 RECODE.</stroke>]]
