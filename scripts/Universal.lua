...
local SwearDetection = loadstring(requesturl("Engine/lib/swear-detection.lua"))()
...
do 
    ...
    local smoothaimpart = {["Value"] ="Pelvis"}
    local smoothaimheld = {["Value"] = "Mouse 1"}

    smoothaim = GuiLibrary["Objects"]["CombatWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Legitbot",
        ["Function"] = function(callback) 
            if callback then 
                BindToStepped("Humanize", function() 
                    local aimpart = smoothaimpart["Value"] == "Pelvis" and "HumanoidRootPart" or "Head"
                    local plr = getPlrNearMouse(smoothaimfov["Value"] * 10)
                    if plr and canBeTargeted(plr, true) and UIS:IsMouseButtonPressed(smoothaimheld["Value"] == "Mouse 1" and 0 or 1) then 
                        aimAt(plr.Character[aimpart].Position, smoothaimsmoothness["Value"])
                    end
                end)
            else
                UnbindFromStepped("Humanize")
            end
        end
    })
...
do 
    ...
    Ragebot = GuiLibrary["Objects"]["CombatWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Ragebot",
        ["Function"] = function(callback) 
            if callback then 
                local Players = game:GetService("Players")
...
do 
    local HitboxExtender = {["Enabled"] = false}

    HitboxExtender = GuiLibrary["Objects"]["CombatWindow"]["API"].CreateOptionsButton({
...
do 
    ...
    ServerCrasher2 = GuiLibrary["Objects"]["ExploitsWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Server Crasher 2",
        ["Function"] = function(callback) 
            if callback then 
-- variables
...
do 
    ...
    ServerCrasher = GuiLibrary["Objects"]["ExploitsWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Server Crasher",
        ["Function"] = function(callback) 
            if callback then 
...
--[[do 
    ...
    spammer = GuiLibrary["Objects"]["MiscellaneousWindow"]["API"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                if spammerfile["Value"] ~= "" and isfile("Engine/"..spammerfile["Value"]) then
                    messagetable = readfile(("Engine/"..spammerfile["Value"]))
                ...
                elseif spammer["Enabled"] then
                    ...
                    fprint("Engine/"..value.." is not a valid file, please make sure it is in your workspace folder and you include the file extension")
                end
            end
        end
    })
...
do 
    local autowalk = GuiLibrary["Objects"]["WorldWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Walkbot",
        ["Function"] = function(callback) 
            if callback then 
                BindToRenderStepped("Walkbot", function()
                    if isAlive() then
                        lplr.Character.Humanoid:Move(Vector3.new(0, 0, -1), true)
                    end
                end)
            else
                UnbindFromRenderStepped("Walkbot")
            end
        end
    })
end
...
do
    ...
    speed = GuiLibrary["Objects"]["MovementWindow"]["API"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                BindToHeartbeat("Speedhack", function(dt)
                    if isAlive() then
...
do
    ...
    local function hj() 
        ...
        elseif HighJumpMode["Value"] == "Instant" then
...
do 
    ...
    GuiLibrary.SaveConfig = function(name, isAutosave) 
        local path = ("Engine/configs/"..tostring(shared.FuturePlaceId or game.PlaceId).."/"..name..".search.json")
...
do 
    ...
    breadcrumbs = GuiLibrary["Objects"]["WorldWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Walkpath",
        ["Function"] = function(callback) 
            --// line mode:
...
do 
    local HumanoidESP = {["Enabled"] = false}

    HumanoidESP = GuiLibrary["Objects"]["RenderWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Humanoid ESP",
        ["Function"] = function(callback) 
            if callback then 
                _G.Configuration = {
...
do 
    local ChatSpam = {["Enabled"] = false}

    ChatSpam = GuiLibrary["Objects"]["MiscellaneousWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Chat Spam",
        ["Function"] = function(callback) 
            if callback then 
                getgenv().chatspam = true

                while chatspam do wait(2)
                local A_1 = "valkyrie.uno best roblox script" local A_2 = "All" 
...
do
    local function addtofile(plr, reason, result) 
        if not betterisfile("Engine/reported.txt") then 
            writefile("Engine/reported.txt", "-- Valkyrie AutoReport logs.\n-- by stryder\n\n--LOG BEGIN --\n")
        end

        appendfile("Engine/reported.txt", "reported "..plr.Name.." ("..tostring(plr.UserId)..") for "..reason.." for saying '"..result.."'\n")
    end
